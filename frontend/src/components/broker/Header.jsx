        ...
        <header className="bg-white w-full fixed top-0 md:static shadow-sm border-b border-gray-200 h-16 flex items-center justify-between px-4 md:px-6 z-20">
            ...
            <div className="flex-1 max-w-lg flex justify-end md:justify-start px-2">
                ...
                {/* {
                    ...
                    :
                    <button className="flex items-center gap-2 bg-primary text-white px-5 py-2 rounded-md cursor-pointer" onClick={() => navigate(`/broker/dashboard`)}><RefreshCcw size={20} /> Sell</button>
                } */}
            </div>
            ...
            <div className="flex items-center space-x-4 md:space-x-5">
                ...
                <div className="flex items-center space-x-3 pl-3 border-l border-gray-200">
                    ...
                    </div>
                    {
                        user?.image
                            ?
                            <Link to={`/broker/profile`}><img src={user?.image} alt="userImage" className="h-10 w-10 rounded-full" /></Link>
                            :
                            <Link to={`/broker/profile`} className="h-10 w-10 rounded-full bg-gradient-to-r from-orange-500 to-orange-600 flex items-center justify-center text-white font-semibold">
