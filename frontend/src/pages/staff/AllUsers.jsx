...
function AllUsers() {
    ...
    const handleVerifyIdentity = async () => {
        ...
        try {
            const { data } = await axiosInstance.patch(
                ...
                {
                    notes: `Verified by staff on ${new Date().toLocaleString()}`
                }
            );
    ...
    const getUserTypeBadge = (type) => {
        const config = {
            ...
            staff: { color: "bg-yellow-100 text-yellow-800", text: "Staff" },
            cashier: { color: "bg-gray-100 text-gray-800", text: "Cashier" },
        };
